...
namespace Hotel_Listing.Controllers {
    ...
    public class HotelController : ControllerBase {
        ...
        [HttpGet]
        [Route("[controller]/GetHotels")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> GetHotels() {
            try {
                var hotels = await _unitOfWork.Hotels.GetAll();
                var results = _mapper.Map<IList<HotelModel>>(hotels);
                return Ok(results);
            }
            catch (Exception e) {
        ...
        [HttpGet]
        [Route("[controller]/GetHotel")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> GetHotel([Required] int id) {
            try {
                var hotel = await _unitOfWork.Hotels.Get(q => q.Id == id, new List<string> { "Country" });
                var result = _mapper.Map<HotelModel>(hotel);
                return Ok(result);
            }
            catch (Exception e) {
